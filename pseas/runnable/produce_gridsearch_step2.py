...
Parameter space of methods is defined after line 100.
"""
...
argument_parser.add_argument('-o', '--output-suffix',
                             ...
                             help="CSV data filename suffix (default: '[scenario]_[nb configurations]_[ratio instance]_[method optimized]')"
                             )
...
if os.path.exists(f"./grid_search_selections_{output_suffix}.csv"):
    original_df_general = pd.read_csv(f"./grid_search_selections_{output_suffix}.csv")
...
if original_df_general is not None:
    df_tmp = original_df_general.append(df_tmp)
df_tmp.to_csv(f"./grid_search_selections_{output_suffix}.csv")
